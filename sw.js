const CACHE='tino-travel-v10.9.1';
const LEGACY_AUTO_ACTIVATE=['tino-travel-v10.8.1','tino-travel-v10.9.0'];
const LOCAL=['./','./index.html','./styles.css','./app.js','./itinerary-core.js','./data.js','./manifest.json','./icon.svg'];
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(LOCAL)).then(()=>caches.keys()).then(keys=>{if(keys.some(key=>LEGACY_AUTO_ACTIVATE.includes(key)))return self.skipWaiting();})));
